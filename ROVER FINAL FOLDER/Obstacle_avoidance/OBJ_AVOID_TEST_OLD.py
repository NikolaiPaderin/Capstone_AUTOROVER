...
angle_difference = 0
deference = 0
avoid = case_classification()
while(distance_meters > 0):
    if(deference > 0):
        execute_command('left', deference)
        deference = 0
    if(deference < 0):
        execute_command('right', deference)
        deference = 0
    ...
    if(angle_difference < 0):
        ...
        curr_angle = angle_from_north
    avoid = case_classification()
    while(avoid != 'straight'):
        while(avoid == 'left'):
            execute_command('left', 15)
            avoid = case_classification()
            deference += 15
        while(avoid == 'hard left'):
            execute_command('left', 30)
            avoid = case_classification()
            deference += 30
        while(avoid == 'right'):
            execute_command('right', 15)
            avoid = case_classification()
            deference -= 15
        while(avoid == 'hard right'):
            execute_command('right', 30)
            avoid = case_classification()
            deference -= 30
